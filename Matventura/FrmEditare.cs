    ...
    public partial class FrmEditare : Form
    {
        ...
        private void lbIntrebari_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                tbrLupta.Text = lbID_Lupta.Items[lbIntrebari.SelectedIndex].ToString();
        ...
        private void pbAdaugare_Click(object sender, EventArgs e)
        {
            OleDbConnection con = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data source=" + cale + "\\Soft.accdb");
            con.Open();
            string qs = "Insert into Intrebari (Exercitiu,R1,R2,R3,RC,ID_Lupta) values(@int,@R1,@R2,@R3,@RC,@ID_Lupta)";
            OleDbCommand com = new OleDbCommand(qs, con);
            com.Parameters.AddWithValue("Exercitiu", tbIntrebare.Text);
            com.Parameters.AddWithValue("R1", tbr1.Text);
            com.Parameters.AddWithValue("R2", tbr2.Text);
            com.Parameters.AddWithValue("R3", tbr3.Text);
            com.Parameters.AddWithValue("ID_Lupta",tbrLupta.Text);
            if (rbA.Checked == true)
            {
                com.Parameters.AddWithValue("RC", tbr1.Text);
            }
            if (rbB.Checked == true)
            {
                com.Parameters.AddWithValue("RC", tbr2.Text);
            }
            if (rbC.Checked == true)
            {
                com.Parameters.AddWithValue("RC", tbr3.Text);
            }
        ...
        private void pictureBox3_Click(object sender, EventArgs e)
        {
            ...
            OleDbConnection con = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data source=" + cale + "\\Soft.accdb");
            con.Open();
            string qs = "Update Intrebari SET Exercitiu=@exercitiu,R1=@R1,R2=@R2,R3=@R3,RC=@RC where ID=@ID";
            OleDbCommand com = new OleDbCommand(qs, con);
            com.Parameters.AddWithValue("Exercitiu", tbIntrebare.Text);
            com.Parameters.AddWithValue("R1", tbr1.Text);
            com.Parameters.AddWithValue("R2", tbr2.Text);
