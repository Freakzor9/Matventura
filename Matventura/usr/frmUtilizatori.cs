...
namespace Imperiul_Britanic
{
    public partial class frmUtilizatori : Form
    {
        ...
        private void clase()
        {
            ...
            con.Open();
            string text = "select Distinct Clasa from Useri";
        ...
        private void schimbareCombo(int slct)
        {
            ...
            if (slct == 0)
            {
                text = "select * from Useri";
            }
            else
            {
                text = "select * from Useri where Clasa='" + comboBox1.Items[slct].ToString()+"'";
            }
            OleDbCommand com = new OleDbCommand(text, con);
            OleDbDataReader r = com.ExecuteReader();
            while (r.Read())
        ...
        private void txtSave_Click(object sender, EventArgs e)
        {
            OleDbConnection con = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data source=" + cale + "\\Soft.accdb");
            con.Open();
            string qs = "Update Useri SET Acceptat=@acceptat where Mail=@mail";
            ...
            if (rbA.Checked == true)
            {
                ...
                com.Parameters.AddWithValue("Email", lstNume.Items[lstUseri.SelectedIndex]);
            }
            else
            {
